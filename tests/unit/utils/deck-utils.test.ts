...
describe('deck-utils', () => {
	...
	describe('splitDeckPath', () => {
		...
		it('should split a nested deck path', () => {
			expect(splitDeckPath('Maths::Linear algebra')).toEqual([
				'Maths',
				'Linear algebra',
			]);
		});
		...
		it('should split a deeply nested deck path', () => {
			expect(splitDeckPath('Maths::LA::Matrices')).toEqual([
				'Maths',
				'LA',
				'Matrices',
			]);
	...
	describe('getParentDecks', () => {
		...
		it('should return all parents for a deeply nested deck', () => {
			expect(getParentDecks('Maths::LA::Matrices')).toEqual([
				'Maths',
				'Maths::LA',
			]);
		});
	...
	describe('matchesDeckFilter', () => {
		...
		it('should return true for grandchild of filter', () => {
			expect(matchesDeckFilter(['Maths::LA::Matrices'], 'Maths::LA')).toBe(
				true,
			);
		});

...
import type { Stats, DailyProgress, ReviewSession } from "@/schemas/statistics";
import { EventData, EventType, QueueInitEvent, ReviewFlashcardEvent, SessionEndEvent } from "@/types/events";
import { Logger } from "@/utils/Logger";
import { State } from "ts-fsrs";
import { EventListener } from "./EventListener";

export class StatisticsListener extends EventListener {
  protected readonly eventTypes = [EventType.ReviewFlashcard, EventType.QueueInit, EventType.SessionEnd];

  constructor(
    private _statsAdapter: StatisticsAdapter,
  ) {
    super();
  }
  ...
  private handleReview(event: ReviewFlashcardEvent): void {
    ...
    dailyProgress.retention_rate = dailyProgress.total_count > 0
      ? dailyProgress.correct_count / dailyProgress.total_count
      : 0;
  ...
  private handleSessionEnd(event: SessionEndEvent): void {
    const stats = this._statsAdapter.data;
    const { session_id, review_type, date, start_time, end_time, total_count, correct_count, incorrect_count, duration_s } = event.data;
    ...
    const totalReviewed = stats.sessions.reduce((sum: number, s: ReviewSession) => sum + s.total_count, 0);
    const totalCorrect = stats.sessions.reduce((sum: number, s: ReviewSession) => sum + s.correct_count, 0);
  ...
  private updateStreaks(stats: Stats): void {
    ...
    const sortedDays = Object.keys(stats.progress)

...
import {
	...
	FlashcardAdapterSetEvent,
	FlashcardAdapterUpdateEvent,
} from '../../domains';
...
export class FlashcardAdapterSetHandler extends EventHandler<FlashcardAdapterSetEvent> {
	constructor(deps: IEventRegistryDependencies) {
		super(deps);
	}

	async handle(event: FlashcardAdapterSetEvent): Promise<void> {
		const adapter = this._adapters.get(AdapterKey.flashcard)! as FlashcardAdapter;
		adapter.set(event.data);
	}
}

export class FlashcardAdapterUpdateHandler extends EventHandler<FlashcardAdapterUpdateEvent> {
	constructor(deps: IEventRegistryDependencies) {
		super(deps);
	}

	async handle(event: FlashcardAdapterUpdateEvent): Promise<void> {
		const adapter = this._adapters.get(AdapterKey.flashcard)! as FlashcardAdapter;

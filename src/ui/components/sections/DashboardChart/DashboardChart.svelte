<script lang="ts">
	import type { DashboardChartProps } from './DashboardChart.types';
	...
	const barPadding = 8;
	let barWidth = $derived(chartWidth / Math.max(stats.progressData.length, 1) - barPadding);
	let maxCompleted = $derived(
		Math.max(...stats.progressData.map((d) => d.completed), stats.dailyGoal, 1),
	);
	let goalY = $derived(chartHeight - (stats.dailyGoal / maxCompleted) * chartHeight);
		...
		>
			...
			{#each stats.progressData as entry, i}
				...
				<rect
					...
					height={h}
					rx="4"
					fill={entry.completed >= entry.target

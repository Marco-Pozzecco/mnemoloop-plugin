...
import { IRecoveryNotifier } from './utils/contract';
	...
	notifyRecoveryComplete(result: IndexRecoveryResult): void {
		if (result.success) {
			if (result.cards_failed === 0) {
				new Notice(
					`Knowledge Accelerator: Rebuild complete. ${result.cards_recovered} cards recovered in ${Math.round(result.duration_ms)}ms.`,
				);
			} else {
				new Notice(
					`Knowledge Accelerator: Rebuild complete with issues. ${result.cards_recovered} recovered, ${result.cards_failed} failed. Check console for details.`,
				);
			}
		} else {
			new Notice(
				'Knowledge Accelerator: Critical error during index rebuild. Please check console.',
			);
		}
	}

...
import { Index, SyncResult, SyncConflict, ISyncNotifier } from './types';
...
export class YamlJsonSync {
	...
	detectConflict(jsonMetadata: FlashcardMetadata, yamlMetadata: FlashcardMetadata): SyncConflict | null {
		...
		if (jsonUpdated === yamlUpdated) {
			if (JSON.stringify(jsonMetadata.srs) === JSON.stringify(yamlMetadata.srs) && 
	...
	async syncJsonToYaml(uuid: string): Promise<SyncResult> {
		...
		if (!jsonMetadata) {
			return { success: false, conflicts_resolved: 0, errors: [`Flashcard with UUID ${uuid} not found in index`] };
		}
		...
		try {
			...
			const errorMsg = error instanceof Error ? error.message : 'Unknown error during syncJsonToYaml';
	...
	async syncYamlToJson(filePath: string): Promise<SyncResult> {
		try {
			...
			if (!parseResult.success || !parseResult.metadata) {
				return { success: false, conflicts_resolved: 0, errors: [parseResult.error || 'Failed to parse YAML'] };
			}
			...
			const errorMsg = error instanceof Error ? error.message : 'Unknown error during syncYamlToJson';
			this.notifier?.logError(errorMsg);
			return { success: false, conflicts_resolved: 0, errors: [errorMsg] };
		}
	}
	...
	private generateYaml(metadata: FlashcardMetadata): string {
		...
		if (metadata.srs.elapsed_days !== undefined) lines.push(`srs_elapsed_days: ${metadata.srs.elapsed_days}`);
		if (metadata.srs.scheduled_days !== undefined) lines.push(`srs_scheduled_days: ${metadata.srs.scheduled_days}`);
		if (metadata.srs.learning_steps !== undefined) lines.push(`srs_learning_steps: ${metadata.srs.learning_steps}`);
